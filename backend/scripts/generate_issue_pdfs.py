...
def make_scenario_1_weight_container(seed: int) -> dict[str, bytes]:
    ...
    scenario = ShipmentScenario(
        truth=truth, scenario_kind="weight_container_mismatch", packing_list_overrides=pl_overrides
    )
...
def make_scenario_2_incoterm_value(seed: int) -> dict[str, bytes]:
    ...
    inv_overrides = {
        "incoterm": "FOB",
        "total_value": sum(li.quantity * li.unit_price for li in truth.line_items)
        + 1500.0,  # line items won't sum correctly  # noqa: E501
    }
    bol_overrides = {
